...
- Додаток: `C:\Booking`
- Процес: НЕ pm2! Запускається через `C:\Booking\watchdog.bat` (loop: node → crash → 5с → restart)

## Деплой бота на Windows Server (src/)
Зміни в `src/` НЕ деплояться через Railway — треба вручну на сервер:
```python
import paramiko, time
client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect('185.203.242.10', username='Administrator', password='7ow1s82cM41L', timeout=30)
# 1. Pull новий код
stdin, stdout, stderr = client.exec_command('cd C:\\Booking && git pull origin main', timeout=60)
print(stdout.read().decode())
# 2. Вбити node-процес — watchdog.bat перезапустить через 5с
stdin, stdout, stderr = client.exec_command('taskkill /IM node.exe /F', timeout=10)
print(stdout.read().decode())
time.sleep(8)
# 3. Перевірити що процес піднявся
stdin, stdout, stderr = client.exec_command('tasklist | findstr /i node', timeout=10)
print(stdout.read().decode())
client.close()
```
Архітектура деплою:
- `src/` (бот) → git push → SSH на сервер → git pull + taskkill node.exe → watchdog перезапускає
- `app/` (міні-апп) → git push → Railway автодеплой (окремо, не зачіпає бота)
